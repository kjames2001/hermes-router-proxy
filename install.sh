...
install_file "$SCRIPT_DIR/trace.py" "$ROUTER_DIR/trace.py" "trace.py"
install_file "$SCRIPT_DIR/surrogate_models.py" "$ROUTER_DIR/surrogate_models.py" "surrogate_models.py"
...
echo ""
echo "  1. On the very first request, the router will read your USER.md"
echo "     and extract a profile hint via the flash classifier model."
echo "     (~3 seconds on first call, cached permanently after)"
echo ""
echo "  2. Every new session gets classified once into a task category"
echo "     (chat, code, devops, research, homeassistant — configurable)."
echo "     Each category maps to its own model + fallback chain."
echo ""
...
echo ""
echo "  4. Manual override: type /use:<category> to force a category."
echo "     e.g. /use:code Write a Python script..."
echo ""
